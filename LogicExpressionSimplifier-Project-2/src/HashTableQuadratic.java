...
public class HashTableQuadratic {
	...
	public void QuadraticHash(ArrayList<String> wordsList,ArrayList<File> fileList,ArrayList<Integer> wordCountList){
		int addition=0;
	...
	public void hashingMain(String wordToHash,boolean flag){
		...
		if(flag==true){
			while(!insert(wordToHash,slotKey,hashUniqKey)){
				i++;
				collision_counter++;
				slotKey=Math.abs((func1(hashUniqKey)+2*(i)+3*(i*i))%tableSize);
			}
			}	
	}
	...
	public int getPrime(int minNumber){////slotKey=Math.abs((func1(hashUniqKey)+2*(i)+3*(i*i))%tableSize);
	...
	public void search(String[] tokens, ArrayList<Integer> tokensQuantity){
		...
		for (int i = 0; i < tokens.length; i++) {
			...
			if(hashTable[slotKey]!=null&&hashTable[slotKey].getWord().equalsIgnoreCase(tokens[i])){
				...
				while(flag==false){
					j++;
					slotKey=Math.abs((func1(hashUniqKey)+2*(j)+3*(j*j))%tableSize);
					if(hashTable[slotKey]!=null&&hashTable[slotKey].getWord().equalsIgnoreCase(tokens[i])){
						arrayTokenSlotKey[i]=slotKey;
						flag=true;
					}
